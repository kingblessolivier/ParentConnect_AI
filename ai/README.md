...
| `service/pipeline.py` | **Coach pipeline** — assembles crisis → refusal → retrieval → grounding gate → generation → citation check into a `CoachResult` (docs/ai/ai-architecture.md). Generation is behind a `Generator` seam (no LLM wired yet, ADR-0004), so the grounded path returns a safe "I don't know" until a model is configured |
| `service/app.py` | **FastAPI `POST /coach`** — the HTTP endpoint the Node backend calls; response matches the Node `AiCoachResult` contract (camelCase). No PII received (ADR-0004) |
| `service/main.py` | uvicorn entrypoint (`uvicorn parentconnect_ai.service.main:app`); default pipeline has an empty corpus + no generator until real content/model land |

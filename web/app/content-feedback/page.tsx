...
function Stars({ value }: { value: number | null }) {
  ...
  return (
    <span>
      ...
      <span className="muted">{value.toFixed(1)}</span>
    </span>
  );
}
      ...
      {[5, 4, 3, 2, 1].map((star) => {
        ...
        return (
          <div key={star} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
            <span className="muted" style={{ width: 12, fontSize: 12 }}>{star}</span>
            <div className="bar" style={{ flex: 1 }}><span style={{ width: pct }} /></div>
            <span className="muted" style={{ width: 34, fontSize: 12, textAlign: 'right' }}>{n}</span>
          </div>
        );
      })}
    </div>
            ...
            {rows.map((r) => (
              <tr key={r.itemId}>
                <td>
                  ...
                  <div className="muted" style={{ fontSize: 12 }}>{r.itemId}</div>
                </td>

...
- **Editable referral directory** (`modules/safeguarding`, FR-33): `GET/POST /api/v1/admin/referral-directory`, `PUT/DELETE /api/v1/admin/referral-directory/:id` (admin-only, each change audited). The DB table is an **override layer over the file bundle**, not a replacement: overrides are served when present, the `referral-directory.json` baseline otherwise — **and the baseline again if storage is unreachable**. So an admin can correct a district's contacts without a redeploy, removing the last override is safe (the baseline takes over), and the referral pathway never acquires a hard DB dependency (FR-21, NFR-06, ADR-0010). Postgres-backed (`migrations/0009_referral_directory.sql`, `pg-mem`-tested).
- **System-wide audit log** (`modules/audit`, NFR-11): `GET /api/v1/admin/audit?action=&entity=&actorId=&since=&limit=` — admin-only, read-only, newest first. Records role changes (FR-33), editorial transitions (FR-20), referral status changes (FR-23), and erasures (NFR-17), each as `actor + action + entity + timestamp`. **Immutability is structural:** `AuditRepository` exposes only `append` and reads — no update or delete method exists for any caller to reach for, and there is no write endpoint (events come from the modules that perform the action, never from a client). **Privacy (NFR-10/15):** metadata is sanitised through `lib/redact.ts`, capped, and coerced to short strings — a referral's note is never copied into the log, because it may describe a disclosure.
  - **Deployment hardening:** grant the app's DB role `INSERT`/`SELECT` on `audit_events` only — `REVOKE UPDATE, DELETE ON audit_events FROM <app_role>;`. This lives with provisioning rather than in a migration because it depends on the provider's role layout (Q1 hosting still open).
- **Retention job** (`modules/retention`, NFR-19): `npm run retention` applies `docs/compliance/retention-schedule.md` on a schedule (cron/worker) and audit-logs **counts only, never content**. The policy lives as data in `policy.ts` so the job and the DPO-facing schedule argue from the same numbers. Two guardrails: records the schedule marks `retain` (referrals — legal duty; audit events — immutable) have **no code path that deletes them**, and the run **reports which rules it could not enforce** (message bodies aren't persisted yet), so "nothing expired" is never confused with "not actually enforced".

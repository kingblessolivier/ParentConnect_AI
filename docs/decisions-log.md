...
| V6 | Teenage-pregnancy prevalence statistics for Rwanda | **PARTIALLY VERIFIED** (2026-08-14) — **Rwanda DHS 2025 (RBC): 8% of girls aged 15–19 have ever been pregnant**, up from ~5% in 2019/20; rising from <1% at age 15 to nearly 20% by 19. Accessed via secondary reporting, so `[VERIFY]` against the primary RDHS publication before external/funder use. | `vision.md` |
...
| Date | Change |
|---|---|
| 2026-08-15 | **[VERIFY]** Extends the entry below: every `site/lib/kb.ts` chunk (all 16) now carries a one-line bilingual `followUp` suggested question, shown as a chip after the coach answers (`lib/coach.ts`'s `nextSuggestions()`). The 16 Kinyarwanda follow-up questions are, like the passages below, assistant-written and not native-reviewed — short phrases, lower risk than full paragraphs, but still pending native-speaker review before this is treated as launch-representative. |
| 2026-08-15 | **[VERIFY]** Six new Kinyarwanda passages added to `site/lib/coach.ts` (`DEMO_KB`, the no-key demo's canned replies for kb-parent-role/kb-fear/kb-start-early/kb-dont-know/kb-not-accusing/kb-fathers, mirroring the English drafted this session in `ai/corpus/seed/parent-communication.yaml`). Written by the assistant, not a native speaker — same "illustrative, pending review" status as the rest of this file's Kinyarwanda content, but flagged explicitly here since it's new prose (not a keyword) and more likely to carry a phrasing or grammar error than the shorter keyword additions alongside it. Needs native-speaker review before this demo is treated as launch-representative. |
| 2026-08-14 | **Seed corpus drafted from published research** (`ai/corpus/seed/`) — six parent–adolescent communication entries grounded in the WHO CSE fact sheet (11 Mar 2026) and a 2025 Rwanda Journal of Medicine and Health Sciences study of 132 adolescents in Nyarugenge/Kamonyi. **All `status: draft`; none is approved or servable.** Scope deliberately limited to *communication* guidance — clinical SRH content still requires a clinician (Q4), and prescribing/diagnosis/termination remain refused outright (NFR-21). The loader forces DRAFT status and rejects any entry without a named source, so research cannot bypass review (FR-20). |
| 2026-08-14 | **V6 partially verified**: Rwanda DHS 2025 (RBC) reports 8% of girls 15–19 ever pregnant (was ~5% in 2019/20). Confirm against the primary RDHS publication before citing to funders. |
| 2026-07-30 | **[ASSUMPTION]** `site/`'s hero uses a real photo (landscape, Rwanda, golden hour — `public/hero-rwanda.jpg`, free Unsplash License, credited in `site/README.md`) instead of stock photography of people. Rationale: a stock photo of "a Rwandan parent and teenager" would imply those are real programme participants when they aren't — inconsistent with a child-safeguarding brand that otherwise never fabricates who's in the system. A landscape carries no identity/consent risk at all. If real photography of people is wanted later, it should come from the actual programme (CHWs, community sessions) with proper consent, not a stock library. |
| 2026-07-30 | **[ASSUMPTION]** New top-level app `site/` (Next.js, ADR-0016 stack) added as the **public** informational site — distinct from `web/` (staff console, authenticated) and `mobile/` (the parent-facing app). Rationale: audience (public, unauthenticated), design treatment (editorial/marketing vs. operational dashboard), and bandwidth posture (must stay light for a low-connectivity visitor) are different enough from `web/`'s stated staff-only contract that extending `web/` would have blurred that boundary. Deliberately prints **no specific helpline/referral phone numbers** (V2 is still `[VERIFY]`, and correct numbers are per-district) — the site describes the capability and points to the in-app/SMS referral directory instead. |
| 2026-07-30 | **[ASSUMPTION]** The first `admin` account cannot be created through the API — `POST /api/v1/admin/users/:id/role` requires an existing admin (chicken-and-egg by design, so no unauthenticated path can ever mint an admin). The very first admin per deployment must be seeded directly (migration or a one-off DB update against the `parents` table), not through a code path. Implemented alongside `backend/src/modules/identity` admin user-list/role-assignment endpoints (FR-33). A documented seed/bootstrap script is a reasonable follow-up if this proves error-prone operationally. |
